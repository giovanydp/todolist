...
// Update status tugas menjadi terlambat jika melewati tenggat
mysqli_query($db, "UPDATE tugas SET status = 2 WHERE status = 0 AND tenggat < '$today' AND user_id = $user_id");

if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['tugas_id'])) {
    $tugas_id = $_POST['tugas_id'];
    $status = 1; // Set status jadi selesai
...
<div class="wadah">
        ...
        <?php while ($row = mysqli_fetch_assoc($result_tugas)): ?>
            <form method="POST" class="tugas">
                ...
                <ul>
                    ...
                    ?>
                        <li>
                            <input type="checkbox" onchange="updateSubtugas(<?= $subtugas['id'] ?>, this)" <?= $subtugas['status'] == 'selesai' ? 'checked' : '' ?>>
    ...
    <div class="daftar-tugas terlambat">
        <?php while ($row = mysqli_fetch_assoc($result_terlambat)): ?>
            <form method="POST" class="tugas">
                <input type="hidden" name="tugas_id" value="<?= $row['id']; ?>">
                <input type="checkbox" checked disabled name="status" onchange="this.form.submit()">
                <div class="info-tugas">
                    <h3><?= htmlspecialchars($row['nama_tugas']); ?></h3>
                    <p><?= nl2br(htmlspecialchars($row['deskripsi'])); ?></p>
                    <span class="tenggat">Tenggat: <?= htmlspecialchars($row['tenggat']); ?></span>
